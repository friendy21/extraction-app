...
export default function DashboardPage() {
  ...
  const { data: glynacScore, isLoading: isLoadingScore } = useGetGlynacScoreQuery()
  const { data: riskAlerts, isLoading: isLoadingAlerts } = useGetRiskAlertsQuery()
  const { data: sentimentData, isLoading: isLoadingSentiment } = useGetSentimentAnalysisQuery()
  const { data: workloadData, isLoading: isLoadingWorkload } = useGetWorkloadAnalysisQuery()
  const { data: fileActivity, isLoading: isLoadingFiles } = useGetFileActivityQuery()
  const { data: employees, isLoading: isLoadingEmployees } = useGetEmployeeInsightsQuery()
  
  // Fetch details data (only when needed)
  const { data: employeeDetails, isLoading: isLoadingEmployeeDetails } = useGetEmployeeDetailsQuery(
    selectedEmployeeId || '', 
    { skip: !selectedEmployeeId || !employeeModalOpen }
  )
  
  const { data: alertDetails, isLoading: isLoadingAlertDetails } = useGetAlertDetailsQuery(
    selectedAlertId || '', 
    { skip: !selectedAlertId || !harassmentModalOpen }
  )
